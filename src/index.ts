...
if (!initPix.client_id || !initPix.client_secret) {
    console.error('Erro: CLIENT_ID e/ou CLIENT_SECRET não definidos no arquivo .env');
    process.exit(1);
}
    ...
    (async () => {
        try {
            ...
            if (!valor || isNaN(parseFloat(valor))) {
                return res.status(400).json({ error: 'Valor inválido. Envie um valor numérico no body.' });
            }
